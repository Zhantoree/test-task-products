...
export const ProductList: FC<ProductListProps> = ({productsData}) => {
    ...
    return (
        <section className="w-full px-4">
            <div className="max-w-[708] mx-auto rounded-xl mb-10 bg-grey-300">
                <p className="text-4xl text-center">Добавленные товары</p>
            </div>

            <div className="max-w-[983] m-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 gap-y-10">
                {products?.map(product => (
                    <Card key={product?.id} product={product} amount={product?.id ? cart[product?.id] : 0}
